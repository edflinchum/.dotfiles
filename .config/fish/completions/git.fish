## ---------------------------------------------------------------------------
##  MODIFIED from defaults copied from /usr/share/fish/completions/git.fish
##  search 'MODIFIED' to see changed lines.
## ---------------------------------------------------------------------------
...
### BEGIN MODIFIED LINES ###
    # eval command git $global_args \$saved_args 2>/dev/null
    eval git $global_args \$saved_args 2>/dev/null
### END MODIFIED LINES ###
end
...
### BEGIN MODIFIED LINES ###
    ...
    set -l root (__fish_git rev-parse --show-toplevel 2>/dev/null; and __fish_git config --get core.bare 2>/dev/null)
### END MODIFIED LINES ###
    or return
...
### BEGIN MODIFIED LINES ### : Workaround added because `git add` completions take too long with .dotfiles due to searching untracked files in entire home directory
